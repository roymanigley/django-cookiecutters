...
class {{model.name}}(models.Model):
    {% if 'fields' in model.keys() %}{% for field in model.fields %}{{field.name}} = models.{{ field.type }}(
        ...
        {%- if field.type == 'UploadField' -%}upload_to='{{model.name}}_uploads', {%- endif -%}
    )
    {% endfor %}{% endif %}
    {% if 'relations' in model.keys() %}{% for relation in model.relations %}{{relation.name}} = models.ForeignKey(
        '{{ relation.type }}', {% if 'required' in relation.keys() and relation.required == False -%}null=True, {%- endif -%}on_delete=models.DO_NOTHING
    )
    {% endfor %}{% endif %}
    def __str__(self):
